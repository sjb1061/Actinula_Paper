...
homohydra.map<- read.table("Reduced_Ec_Sensory_Percep_Light_Stim_R_output_genes.txt",header=T, stringsAsFactors=F) 
...
keep<-unique(homohydra.map$Actinula_seqid); length(keep) #1 col  
