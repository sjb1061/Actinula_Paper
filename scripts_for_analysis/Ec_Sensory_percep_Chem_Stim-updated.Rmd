...
refblast<-read.table("Homo.fa_ref_blastout_sens_percep_chem_stim ",header=F,stringsAsFactors = F)
head(refblast) #syds: 17
names(refblast)[1:2]<-c("gene_acc","seqid")

#link gene_acc to gene symbol (human)
homo_symbols<-read.table("gene_symbol_accid_sens_percep_chem_stim",header=F,stringsAsFactors = F) 
names(homo_symbols)<-c("symbol","gene_acc") #syds: 212 tot
...
nrow(apo_OGs_acc_sym) #141
head(apo_OGs_acc_sym)
unique(apo_OGs_acc_sym$OG)
#multiple symbols/accs map to single OG, so just for now, just select the first listed
...
unique(apo_OGs_acc_sym$OG) #112 unique OGs with symbols
...
nrow(apo_counts_symbol) #117
head(apo_counts_symbol)



#### Get Species of Interest accids for OGs in gene set ###

#subset melted OG for species of interest
Ec_melted<-subset(meltedOG, grepl("^Ec_actinula",meltedOG$seqid)) #works
#link human apoptosis apo_OGs_acc_sym to hydractinia (human OG to other species seqids)
actinula_acc_apo_OGs_acc_sym<-inner_join(apo_OGs_acc_sym[,1:4],Ec_melted,by="OG") #works
#rename cols:
names(actinula_acc_apo_OGs_acc_sym)[1:5]<- c("gene_acc", "Homo_seqid", "OG", "symbol", "Actinula_seqid")  #works
head(actinula_acc_apo_OGs_acc_sym)
nrow(actinula_acc_apo_OGs_acc_sym) #619

#Get original header for heatmap/salmon #almost
gene_acc_simple<-unlist(strsplit(actinula_acc_apo_OGs_acc_sym$Actinula_seqid,split="..", fixed = T))[c(T,F)] #split on .. and only keep first split objects

actinula_acc_apo_OGs_acc_sym$Actinula_seqid<-gene_acc_simple #change col items to new headers
head(actinula_acc_apo_OGs_acc_sym)

unique(actinula_acc_apo_OGs_acc_sym$OG)#92
unique(actinula_acc_apo_OGs_acc_sym$Actinula_seqid)#92
#hydract_acc_apo_OGs_acc_sym$unique_hydract <-unique(hydract_acc_apo_OGs_acc_sym$OG)
nrow(actinula_acc_apo_OGs_acc_sym) #619

write.table(actinula_acc_apo_OGs_acc_sym, "C:\\Desktop\\R\\Transcriptomics_Actinula_3-17-20\\Reduced_Ec_Sensory_Percep_Chem_Stim_R_output_genes.txt", sep="\t")
